...
public class Ex04_InnerClass {
	...
	//		  외부 클래스를 통해서만 내부 클래스를 생성할 수 있다. (내부 클래스만 별도로 생성 안됨)
	
	// 2. Local : 외부 클래스 메소드 안에 내부 클래스가 존재 
	//			  외부 클래스가 지역 변수처럼 사용
	//			  외부 클래스의 메소드가 호출(실행)될 때 생성할 수 있다.
	//			  지역 변수처럼 외부 클래스의 해당 메소드가 종료되면 내부 클래스도 지워진다.
	...
	//					   어쩔수 없이 내부 클래스의 멤버 변수를 static으로 사용할 때
	...
	//					   다시 호출할 수 없다. (일회용으로 한 번 사용하고 재활용 불가)
}
